  ...
  render() {
    return (
      <View style={styles.root}>
        ...
        {/* <View style={{ height: 1, width: "100%", backgroundColor: "#AAA" }} /> */}
        <Example totalDuration={1200000} trimmerLengthOptionIndex={3} />
      </View>
    );
  }
}
...
class Example extends Component {
  ...
  onSliderValueChanged = (value) => {
    ...
    if (this.trimmerRef && this.trimmerRef.scrollViewRef) {
      ...
      const newScrollPosition =
        (newStartingPosition / totalDuration) *
        this.trimmerRef.state.contentWidth;

      console.log(
        "this.trimmerRef.scrollViewRef.current",
        this.trimmerRef.scrollViewRef
      );
      const node =
        this.trimmerRef.scrollViewRef.current &&
        this.trimmerRef.scrollViewRef.current.getNode();
  ...
  onTrimmerValueChanged = (value) => {
    console.log("onTrimmerValueChanged called");
    ...
    return (
      <View style={styles.exampleRoot}>
        ...
        <View style={styles.sliderContainer}>
          <View style={{ flex: 0 }}>
            <TrimmerLengthButton
              onPress={this.changeTrimmerLength}
              trimmerLengthOption={TRIMMER_LENGTHS[trimmerLengthOptionIndex]}
            />
          </View>
          <View style={{ flex: 1 }}>
            {/* <Slider
            ...
            <View style={styles.timesContainer}>
              <View style={styles.timesInnerContainer}>
                <Text style={styles.timesLabel}>
                  {scrubbing
                    ? formatMilliseconds(startPositionLabel)
                    : formatMilliseconds(startPosition)}
                </Text>
